...
* For PKI you **should** use the Curve22519 option. The RSA option is to be considered to be to simplify key exchange, since you may already hold someones public RSA key.
* **Notice** that using PKI will add the number of changed bits in the image. If you care about detection the following is important:
  * You **should** use relatively small keys for RSA (ideally 2048 bits). See title PKI for details.
...
### Curve22519 key generation
The program can generate a Curve22519 key pair for you using the `generateKey` command. This will create two files, one which ends with ".public.key" which you are to share with anyone you want to communicate with, and one that ends with ".secret.key" which you must keep secret at all times."
...
Notice that the Curve22519 shared key is 256 bits. This means that there will probably be ~128 bits changed. This is considerably smaller than for RSA and should be used to keep detection down.
