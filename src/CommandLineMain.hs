...
doDryEncrypt' imageFile secretFile loops inputFile salt pkiFile signFile = do
  ...
  secretData <- lazyReadOrEmpty secretFile
  inputData <- lazyReadOrEmpty inputFile
  pkiData <- lazyReadOrEmpty $ pkiFile
  signData <- lazyReadOrEmpty signFile
  let publicKey = if pkiData == LC8.empty
                     then Nothing
                     else Just pkiData
  let privateKey = if signData == LC8.empty
                     then Nothing
                     else Just $ PkiSecret signData
  let cryptoState = createDefaultCryptoState (SecretKey secretData) salt loops publicKey privateKey
...
doDecrypt' imageFile secretFile loops output salt pkiFile signFile = do
  ...
  pkiData <- lazyReadOrEmpty pkiFile
  signData <- lazyReadOrEmpty signFile
  let publicKey = if signData == LC8.empty
                     then Nothing
                     else Just signData
  let privateKey = if pkiData == LC8.empty
                     then Nothing
                     else Just $ PkiSecret pkiData
  let cryptoState = createDefaultCryptoState (SecretKey secretData) salt loops publicKey privateKey
                ...
                withNonOption (namedFile "INPUT-FILE") $ \file ->
                  withOption saltOption $ \salt ->
                    withOption pkiFileOption $ \pkiFile ->
                      withOption signatureOption $ \signFile -> io $ doDryEncrypt' image secret loops file (LC8.pack salt) pkiFile signFile
                  ...
                  withOption saltOption $ \salt ->
                    withOption pkiFileOption $ \pkiFile ->
                      withOption signatureOption $ \signFile ->
                        withOption fastMode $ \fastMode' -> io $ doEncrypt' image secret loops file (LC8.pack salt) pkiFile signFile fastMode'
            ...
            withNonOption (namedFile "SHARED-SECRET-FILE") $ \secret ->
              withNonOption Argument.integer $ \loops ->
                withNonOption (namedFile "OUTPUT-FILE") $ \file ->
                  withOption saltOption $ \salt ->
                    withOption pkiFileOption $ \pkiFile ->
                      withOption signatureOption $ \signFile -> io $ doDecrypt' image secret loops file (LC8.pack salt) pkiFile signFile
